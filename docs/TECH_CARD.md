...
| Fast push outbox    | PostgreSQL outbox; send after response; minute cron retry  | Approved      |
...
- Fast push sends one registration per outbox row after the HTTP response; minute cron retries failures.
