...
1. **Answers catalog.** Toon Expo → Mootq uses form `2026-vis-reg-v2` (Armenia landing) or `2026-vis-reg-spyurk-v1` (Spyurk RF `/rf`). Keys are snake_case and flattened (no nested objects). Both catalogs send `form_channel`. Full list: contract Appendix A. Your inbound keys may differ; we will store unknown keys and still accept the ticket. Older stored rows may still use `2026-vis-reg-v1`.
